...
export const useUserStore = defineStore("user", () => {
  ...
  const forgetPassword = async (email) => {
    ...
    try {
      ...
      if (response.data.status === 200) {
        ...
        return {
          success: false,
          message: response.data.message || "Quên mật khẩu thất bại!",
        };
      }
    } catch (err) {
      error.value =
  ...
  const activatePassword = async (email, code) => {
    ...
    try {
      ...
      if (response.data.status === 200) {
        return {
          success: true,
          message: response.data.message || "Kích hoạt mật khẩu thành công!",
        };
      } else {
        return {
          success: false,
          message: response.data.message || "Kích hoạt mật khẩu thất bại!",
        };
      }
    } catch (err) {
      console.error(
        "❌ Lỗi kích hoạt mật khẩu:",
        err.response?.data || err.message
      );
      error.value =
        err.response?.data?.message || "Kích hoạt mật khẩu thất bại!";
      return { success: false, message: error.value };
    } finally {
      loading.value = false;
    }
  ...
  const register = async (username, maSV, password, email, fullName) => {
    ...
    try {
      ...
      if (response.data.status === 200) {
        ...
        return {
          success: false,
          message: response.data.message || "Đăng kí thất bại!",
        };
      }
    } catch (err) {
      error.value =
  ...
  const activateAccount = async (email, otp) => {
    ...
    try {
      ...
      error.value =
        err.response?.data?.message || "Kích hoạt tài khoản thất bại!";
      return { success: false, message: error.value };
    } finally {
      loading.value = false;
    }
  ...
  const login = async (username, password) => {
    ...
    try {
      ...
      const response = await axios.post(
        "https://qldvhumg.up.railway.app/api/Controller_Authenic/Login",
        formData,
        {
          headers: { "Content-Type": "multipart/form-data" },
        }
      ...
      if (response.data.status === 200 && response.data.data) {
        ...
        return {
          success: false,
          message: response.data.message || "Sai tài khoản hoặc mật khẩu!",
        };
      }
    } catch (err) {
      error.value =
  ...
  const updateUserImage = async (urlAvatar) => {
    ...
    try {
      ...
      if (response.data.status === 200) {
        return {
          success: true,
          message: response.data.message || "Cập nhật ảnh thành công!",
        };
      } else {
        return {
          success: false,
          message: response.data.message || "Cập nhật ảnh thất bại!",
        };
      }
    } catch (err) {
      error.value =
